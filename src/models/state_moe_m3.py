...
class StateMoE(GeneralRecommender):
    ...
    def _build_history_context(self, img_latent, txt_latent, item_anchor, img_q, txt_q):
        ...
        img_hist = (txt_rel * txt_latent + item_anchor) / (txt_rel + 1.0).clamp_min(1e-8)
        txt_hist = (img_rel * img_latent + item_anchor) / (img_rel + 1.0).clamp_min(1e-8)
        id_hist = (
...
class StateMoEM3(StateMoE):
    """Three-modality StateMoE variant that preserves the original 2-modality model."""

    def __init__(self, config, dataset):
        super().__init__(config, dataset)

        self.modalities = ["image", "text", "audio"]
        self._qe_ctx_key_map = {"image": "img_q", "text": "txt_q", "audio": "aud_q"}
        self._label_map = {"image": "Img", "text": "Txt", "audio": "Aud"}

        self.aud_hard_clean_corrupt_threshold = (
            float(config["aud_hard_clean_corrupt_threshold"])
            if "aud_hard_clean_corrupt_threshold" in config
            else self.txt_hard_clean_corrupt_threshold
        )
        self.aud_clean_route_margin = (
            float(config["aud_clean_route_margin"])
            if "aud_clean_route_margin" in config
            else self.txt_clean_route_margin
        )
        self.repair_aud_cosine_weight = (
            float(config["repair_aud_cosine_weight"])
            if "repair_aud_cosine_weight" in config
            else self.repair_img_cosine_weight
        )
        self.repair_aud_clean_keep_weight = (
            float(config["repair_aud_clean_keep_weight"])
            if "repair_aud_clean_keep_weight" in config
            else self.repair_img_clean_keep_weight
        )

        self.a_feat = getattr(self, "a_feat", None)
        if self.a_feat is None:
            self.a_feat = self._load_audio_feature_tensor(config)
        if self.a_feat is None:
            raise ValueError("StateMoEM3 expects audio_feature_file for audio modality.")

        if not hasattr(self, "missing_items_a"):
            self.missing_items_a = np.array([], dtype=np.int64)

        self.audio_feat_dim = self.a_feat.shape[1]
        self.register_buffer(
            "_aud_clean_target",
            self._load_optional_repair_target(config, "repair_target_audio_feature_file"),
        )
        self.audio_embedding = nn.Embedding.from_pretrained(self.a_feat, freeze=False).to(
            self.device
        )
        self.audio_adj = self._build_mm_adj(
            self.audio_embedding.weight.detach(),
            self.missing_items_a if self.missing_modal else None,
        )
        self.register_buffer(
            "_aud_raw_snapshot", self.audio_embedding.weight.detach().clone()
        )
        self._gt_audio_state = self._load_audio_gt_state(config)

        self.audio_encoder = nn.Linear(self.audio_feat_dim, self.embedding_dim).to(
            self.device
        )
        self.audio_preference_ = nn.Linear(
            self.embedding_dim, self.embedding_dim, bias=False
        ).to(self.device)
        self.audio_repair_proj = nn.Sequential(
            nn.Linear(self.audio_feat_dim, self.repair_bottleneck),
            nn.ReLU(),
            nn.LayerNorm(self.repair_bottleneck),
        ).to(self.device)
        self.audio_repair_decode = nn.Linear(
            self.repair_bottleneck, self.audio_feat_dim
        ).to(self.device)

        self.audio_encoder.apply(self.init_weight)
        self.audio_preference_.apply(self.init_weight)
        self.audio_repair_proj.apply(self.init_weight)
        self.audio_repair_decode.apply(self.init_weight)

        self.aud_qe = self._build_raw_qe("audio", self.audio_feat_dim)
        if self.freeze_qe:
            self.aud_qe.eval()
            for param in self.aud_qe.parameters():
                param.requires_grad_(False)

        qe_state_dim = self._infer_qe_state_dim(self.img_qe)
        if qe_state_dim != self._infer_qe_state_dim(self.aud_qe):
            raise ValueError("Audio QE state embedding dimension must match image/text.")

        self.aud_keep_expert = KeepExpert(self.repair_bottleneck).to(self.device)
        self.aud_noisy_expert = NoisyRepairExpert(
            self.repair_bottleneck, qe_state_dim
        ).to(self.device)
        self.aud_missing_expert = MissingRepairExpert(self.repair_bottleneck).to(
            self.device
        )
    ...
    def preprocess_missing_modal(self, config):
        ...
        shared_missing = np.asarray(
            self.missing_items.get("all", np.array([], dtype=np.int64)), dtype=np.int64
        )

        def _merge_missing(key):
            specific = np.asarray(
                self.missing_items.get(key, np.array([], dtype=np.int64)), dtype=np.int64
            )
            if shared_missing.size == 0 and specific.size == 0:
                return np.array([], dtype=np.int64)
            return np.unique(np.concatenate((shared_missing, specific))).astype(np.int64)

        self.missing_items_t = _merge_missing("t")
        self.missing_items_v = _merge_missing("v")
        self.missing_items_a = _merge_missing("a")

        missing_union = np.union1d(
            np.union1d(self.missing_items_v, self.missing_items_t), self.missing_items_a
        )
        self.complete_items = np.setdiff1d(np.arange(self.n_items), missing_union)

        if getattr(self, "a_feat", None) is None:
            self.a_feat = self._load_audio_feature_tensor(config)

        def _non_missing(all_items, missing_items):
            if len(missing_items) == 0:
                return all_items
            return np.setdiff1d(all_items, missing_items)

        non_missing_item_t = _non_missing(self.all_items, self.missing_items_t)
        non_missing_item_v = _non_missing(self.all_items, self.missing_items_v)
        non_missing_item_a = _non_missing(self.all_items, self.missing_items_a)
        ...
        if len(self.missing_items_t) > 0:
            self.t_feat[self.missing_items_t] = text_mean
        if len(self.missing_items_a) > 0:
            self.a_feat[self.missing_items_a] = audio_mean

    def _load_audio_feature_tensor(self, config):
        audio_file = config["audio_feature_file"] if "audio_feature_file" in config else None
        if not audio_file:
            return None

        dataset_path = os.path.abspath(config["data_path"] + config["dataset"])
        audio_path = (
            audio_file if os.path.isabs(audio_file) else os.path.join(dataset_path, audio_file)
        )
        if not os.path.isfile(audio_path):
            raise FileNotFoundError("Audio feature file not found: {}".format(audio_path))
        return torch.from_numpy(np.load(audio_path, allow_pickle=True)).float().to(
            self.device
        )

    def _infer_audio_gt_state_file(self, config):
        gt_path = self._infer_gt_state_file(config)
        if gt_path and os.path.isfile(gt_path):
            return gt_path
        ...
        if not basename.startswith("audio_feat_"):
            return None

        candidate = basename.replace("audio_feat_", "gt_state_", 1)
        gt_path = os.path.join(dataset_path, dirname, candidate)
        return gt_path if os.path.isfile(gt_path) else None

    def _load_audio_gt_state(self, config):
        gt_path = self._infer_audio_gt_state_file(config)
        if gt_path and os.path.isfile(gt_path):
            payload = np.load(gt_path, allow_pickle=True).item()
            if "a" in payload:
                return torch.as_tensor(payload["a"], device=self.device, dtype=torch.long)

        audio_state = torch.zeros(self.n_items, device=self.device, dtype=torch.long)
        if self.missing_modal and len(self.missing_items_a) > 0:
            audio_state[
                torch.as_tensor(self.missing_items_a, device=self.device, dtype=torch.long)
            ] = 2
        return audio_state

    def _build_mm_adj(self, feat_weight, missing_indices=None):
        if self.graph_build_device:
            return super()._build_mm_adj(feat_weight, missing_indices)

        feat_weight = feat_weight.detach()
        feat_weight = feat_weight / feat_weight.norm(dim=-1, keepdim=True).clamp_min(1e-12)
        adj = torch.mm(feat_weight, feat_weight.transpose(1, 0))
        adj = build_knn_neighbourhood(adj, topk=self.knn_k)
        if missing_indices is not None and len(missing_indices) > 0:
            adj[missing_indices, :] = 0.0
            adj[:, missing_indices] = 0.0
    ...
    def _qe_key(self, modality):
        return self._qe_ctx_key_map[modality]

    def _get_label(self, modality):
        return self._label_map[modality]

    def _get_missing_idx(self, modality):
        if modality == "image":
            return self.missing_items_v
        if modality == "text":
            return self.missing_items_t
        return self.missing_items_a

    def _get_gt_state(self, modality):
        if modality == "image":
            return self._gt_image_state
        if modality == "text":
            return self._gt_text_state
        return self._gt_audio_state
    ...
    def _get_modal_repair_raw_feat(self, modality):
        ...
        if modality == "text":
            return self._txt_raw_snapshot
        return self._aud_raw_snapshot

    def _get_modal_clean_target(self, modality):
        if modality == "image":
            return self._img_clean_target
        if modality == "text":
            return self._txt_clean_target
        return self._aud_clean_target

    def _get_modal_encoder(self, modality):
        if modality == "image":
    ...
    def _get_modal_preference(self, modality):
        if modality == "image":
            return self.image_preference_
        if modality == "text":
            return self.text_preference_
        return self.audio_preference_

    def _get_modal_repair_proj(self, modality):
        if modality == "image":
            return self.image_repair_proj
        if modality == "text":
            return self.text_repair_proj
        return self.audio_repair_proj

    def _get_modal_repair_decode(self, modality):
        if modality == "image":
            return self.image_repair_decode
        if modality == "text":
            return self.text_repair_decode
        return self.audio_repair_decode

    def _get_modal_qe(self, modality):
        if modality == "image":
            return self.img_qe
        if modality == "text":
            return self.txt_qe
    ...
    def _get_modal_adj(self, modality):
        if modality == "image":
            return self.image_adj
        if modality == "text":
            return self.text_adj
        return self.audio_adj

    def _set_modal_adj(self, modality, adj):
        if modality == "image":
            self.image_adj = adj
        elif modality == "text":
            self.text_adj = adj
    ...
    def _get_modal_experts(self, modality):
        if modality == "image":
            return self.img_keep_expert, self.img_noisy_expert, self.img_missing_expert
        if modality == "text":
            return self.txt_keep_expert, self.txt_noisy_expert, self.txt_missing_expert
        return self.aud_keep_expert, self.aud_noisy_expert, self.aud_missing_expert

    def _get_modal_cosine_weight(self, modality):
        if modality == "image":
            return self.repair_img_cosine_weight
        if modality == "text":
            return self.repair_txt_cosine_weight
        return self.repair_aud_cosine_weight

    def _get_modal_clean_keep_weight(self, modality):
        if modality == "image":
            return self.repair_img_clean_keep_weight
        if modality == "text":
            return self.repair_txt_clean_keep_weight
        return self.repair_aud_clean_keep_weight

    def _compute_qe_ctx(self):
        qe_ctx = {}
        for modality in self.modalities:
            qe = self._get_modal_qe(modality)
            raw_feat = self._get_modal_repair_raw_feat(modality)
            q = self._run_qe(qe, raw_feat)
            if self.ablation_force_uniform_qe:
                q = self._force_uniform_qe_output(q, qe)
            else:
                q = self._apply_missing_override(q, self._get_missing_idx(modality), qe)
            qe_ctx[self._qe_key(modality)] = q
        return qe_ctx

    def _build_history_context(self, latent_dict, item_anchor, q_dict):
        reliability = {
            modality: self._qe_reliability(q_dict[modality]) for modality in self.modalities
        }
        ones = torch.ones_like(next(iter(reliability.values())))

        id_numerator = item_anchor
        id_denominator = ones
        for modality in self.modalities:
            id_numerator = id_numerator + reliability[modality] * latent_dict[modality]
            id_denominator = id_denominator + reliability[modality]
        id_hist = id_numerator / id_denominator.clamp_min(1e-8)

        hist_dict = {}
        for target_modality in self.modalities:
            numerator = item_anchor
            denominator = ones
            for modality in self.modalities:
                if modality == target_modality:
                    continue
                numerator = numerator + reliability[modality] * latent_dict[modality]
                denominator = denominator + reliability[modality]
            hist_dict[target_modality] = numerator / denominator.clamp_min(1e-8)
        return hist_dict, id_hist

    def _mean_other_modal_tensors(self, tensor_dict, target_modality):
        others = [tensor_dict[modality] for modality in self.modalities if modality != target_modality]
        if len(others) == 1:
            return others[0]
        return torch.stack(others, dim=0).mean(dim=0)

    def _aggregate_other_q(self, q_dict, target_modality):
        other_state_emb = self._mean_other_modal_tensors(
            {m: q_dict[m]["state_emb"] for m in self.modalities}, target_modality
        )
        other_severity = self._mean_other_modal_tensors(
            {m: q_dict[m]["severity"] for m in self.modalities}, target_modality
        )
        return {"state_emb": other_state_emb, "severity": other_severity}
    ...
    def _get_clean_route_mask(self, q, modality):
        if modality == "image":
            ...
            margin = self.aud_clean_route_margin
            threshold = self.aud_hard_clean_corrupt_threshold
    ...
    def _encode_modalities(self, modal_feat_dict):
        encoded = {}
        for modality, feat in modal_feat_dict.items():
            encoder = self._get_modal_encoder(modality)
            encoded[modality] = torch.sigmoid(
                self.shared_encoder(self.act_g(encoder(feat)))
            )
        return encoded

    def mge(self):
        raw_feat_dict = {
            modality: self._get_modal_repair_raw_feat(modality) for modality in self.modalities
        }
        qe_ctx = self._get_qe_ctx()
        q_dict = {modality: qe_ctx[self._qe_key(modality)] for modality in self.modalities}
        self._last_qe_ctx = qe_ctx
        if self.ablation_disable_moe_repair:
            self._last_txt_repair_core_feat = raw_feat_dict["text"]
            return self._encode_modalities(raw_feat_dict), raw_feat_dict, q_dict

        latent_dict = {
            modality: self._get_modal_repair_proj(modality)(raw_feat_dict[modality])
            for modality in self.modalities
        }
        item_anchor = self.item_repair_proj(self.item_id_embedding.weight)
        hist_dict, id_hist = self._build_history_context(latent_dict, item_anchor, q_dict)

        repaired_latent_dict = {}
        for modality in self.modalities:
            keep_expert, noisy_expert, missing_expert = self._get_modal_experts(modality)
            repaired_latent_dict[modality] = self._mix_state_experts(
                target_emb=latent_dict[modality],
                other_emb=self._mean_other_modal_tensors(latent_dict, modality),
                item_emb=item_anchor,
                target_hist=hist_dict[modality],
                id_hist=id_hist,
                target_q=q_dict[modality],
                other_q=self._aggregate_other_q(q_dict, modality),
                keep_expert=keep_expert,
                noisy_expert=noisy_expert,
        ...
        self._last_txt_repair_core_feat = None
        for modality in self.modalities:
            raw_feat = raw_feat_dict[modality]
            decoded_feat = self._get_modal_repair_decode(modality)(
                repaired_latent_dict[modality]
            )
            if modality == "text":
                core_repaired_txt_feat = self._apply_text_residual_repair(
                    raw_feat, decoded_feat, q_dict[modality]
                )
                clean_txt_mask = self._get_clean_route_mask(q_dict[modality], "text")
                clean_txt_feat = self._build_clean_text_adapter_feat()
                repaired_feat_dict[modality] = torch.where(
                    clean_txt_mask.expand_as(core_repaired_txt_feat),
                    clean_txt_feat,
                    core_repaired_txt_feat,
                )
                ...
                soft_feat = self._apply_clean_bypass(raw_feat, decoded_feat, q_dict[modality])
                repaired_feat_dict[modality] = self._apply_modality_hard_clean_bypass(
                    raw_feat, soft_feat, q_dict[modality], modality
                )

        return self._encode_modalities(repaired_feat_dict), repaired_feat_dict, q_dict

    def _propagate_modalities(self, item_modal_dict):
        propagated_items = {}
        propagated_users = {}
        user_hist = self.num_inters[: self.n_users]
        item_hist = self.num_inters[self.n_users:]

        for modality in self.modalities:
            preference = self._get_modal_preference(modality)
            item_filter = torch.sparse.mm(
                self.adj.t(), torch.tanh(preference(self.user_embedding.weight))
            ) * item_hist

            item_repr = torch.einsum("ij,ij->ij", item_filter, item_modal_dict[modality])
            for _ in range(self.n_mm_layers):
                item_repr = torch.sparse.mm(self._get_modal_adj(modality), item_repr)

            propagated_items[modality] = item_repr
            propagated_users[modality] = torch.sparse.mm(self.adj, item_repr) * user_hist
        return propagated_items, propagated_users

    def _repair_reconstruction_loss(self, repaired_feat_dict, q_dict):
        loss = torch.tensor(0.0, device=self.device)
        for modality in self.modalities:
            repaired_feat = repaired_feat_dict[modality]
            if modality == "text" and self._last_txt_repair_core_feat is not None:
                repaired_core_feat = self._last_txt_repair_core_feat
            else:
                repaired_core_feat = repaired_feat

            q = q_dict[modality]
            weight = (q["p_noisy"] + q["p_missing"]).detach()
            clean_target = self._get_modal_clean_target(modality)
            if clean_target is not None:
                target = clean_target
                exclude_idx = None
            else:
                target = self._get_modal_repair_raw_feat(modality)
                exclude_idx = self._get_missing_idx(modality)

            loss = loss + self._weighted_repair_loss(
                repaired_core_feat, target, exclude_idx, weight
            )

            cosine_weight = self._get_modal_cosine_weight(modality)
            if cosine_weight > 0.0:
                loss = loss + cosine_weight * self._weighted_cosine_repair_loss(
                    repaired_core_feat, target, exclude_idx, weight
                )

            clean_keep_weight = self._get_modal_clean_keep_weight(modality)
            if clean_keep_weight > 0.0:
                loss = loss + clean_keep_weight * self._weighted_repair_loss(
                    repaired_core_feat,
                    self._get_modal_repair_raw_feat(modality).detach(),
                    None,
                    q["p_clean"].detach(),
                )
        return loss
    ...
    def _router_prior_loss(self):
        ...
        count = 0
        for modality in self.modalities:
            missing_idx = self._get_missing_idx(modality)
            if len(missing_idx) == 0:
                continue
            idx = torch.from_numpy(missing_idx).long().to(self.device)
            q = self._last_qe_ctx[self._qe_key(modality)]
            gate = q["prob"][idx]
            prior = torch.zeros_like(gate)
            prior[:, 2] = 1.0
            total = total + F.kl_div(
    ...
    def calculate_reg_loss(self, user_emb, pos_items_emb, neg_item_emb, modal_emb_dict):
        loss_reg = self.reg_loss(user_emb, pos_items_emb, neg_item_emb) * 1e-5
        for modality in self.modalities:
            loss_reg += self.reg_loss(modal_emb_dict[modality]) * 0.1
        return loss_reg

    def _get_mm_valid_items(self, candidate_items):
        candidate_items = np.asarray(candidate_items, dtype=np.int64)
        if candidate_items.size == 0:
            return candidate_items

        valid_mask = np.ones(candidate_items.shape[0], dtype=bool)
        for modality in self.modalities:
            gt_state = self._get_gt_state(modality)
            if gt_state is None:
                continue
            gt_state_np = gt_state.detach().cpu().numpy()
            valid_mask &= gt_state_np[candidate_items] != 2
        return candidate_items[valid_mask]

    def _compute_pairwise_mm_loss(self, item_modal_dict, item_indices):
        pair_losses = []
        for left_modality, right_modality in combinations(self.modalities, 2):
            pair_losses.append(
                self.info_nce(
                    item_modal_dict[left_modality][item_indices],
                    item_modal_dict[right_modality][item_indices],
    ...
    def _mean_modalities(self, modal_dict):
        return torch.stack([modal_dict[modality] for modality in self.modalities], dim=0).mean(
            dim=0
        )
    ...
    def _compute_extra_diag_lines(self):
        ...
        try:
            self.eval()
            with torch.no_grad():
                _, repaired_feat_dict, q_dict = self.mge()
        finally:
            if was_training:
                self.train()
        ...
        if self.log_qe_state_stats:
            for modality in self.modalities:
                lines.extend(
                    self._build_qe_state_diag_lines(
                        self._get_label(modality),
                        q_dict[modality],
                        self._get_gt_state(modality),
                    )
                )
        ...
        if self.log_repair_feature_stats:
            for modality in self.modalities:
                ...
                repaired_feat = repaired_feat_dict[modality]
                if modality == "text" and self._last_txt_repair_core_feat is not None:
                    repaired_feat = self._last_txt_repair_core_feat.detach()
                else:
                    repaired_feat = repaired_feat.detach()
                lines.extend(
                    self._build_repair_diag_lines(
                        self._get_label(modality),
                        self._get_modal_repair_raw_feat(modality).detach(),
    ...
    def post_epoch_processing(self):
        if not self.log_qe_prob or self._last_qe_ctx is None:
            ...
            base_lines = [
                self._format_qe_prob_line(
                    self._get_label(modality), self._last_qe_ctx[self._qe_key(modality)]
                )
                for modality in self.modalities
            ]
    ...
    def calculate_loss(self, interaction):
        ...
        item_modal_dict, repaired_feat_dict, q_dict = self.mge()

        all_items, _ = torch.unique(
            torch.cat((pos_items, neg_items)), return_inverse=True, sorted=False
        )
        valid_mm_idx = self._get_mm_valid_items(all_items.detach().cpu().numpy())
        loss_mm = self._compute_pairwise_mm_loss(item_modal_dict, valid_mm_idx)

        item_modal_dict, user_modal_dict = self._propagate_modalities(item_modal_dict)
        user_mm = self._mean_modalities(user_modal_dict)
        item_mm = self._mean_modalities(item_modal_dict)
        ...
        loss_reg = self.calculate_reg_loss(
            ...
            item_embedding[pos_items],
            item_embedding[neg_items],
            {modality: item_modal_dict[modality][pos_items] for modality in self.modalities},
        )
        loss_repair = self._repair_reconstruction_loss(repaired_feat_dict, q_dict)
        loss_router = self.loss_weight_router * self._router_prior_loss()
    ...
    def _refresh_repaired_mm_adj(self):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                corrupt_stats = []
                for modality in self.modalities:
                    corrupt_mask = ~self._get_clean_route_mask(q_dict[modality], modality).view(-1)
                    corrupt_mean = float(corrupt_mask.float().mean().item())
                    corrupt_stats.append((modality, corrupt_mask, corrupt_mean))

                if all(
                    mean <= self.mm_adj_refresh_skip_qe_threshold
                    for _, _, mean in corrupt_stats
                ):
                    if hasattr(self, "logger") and self.logger is not None:
                        summary = ", ".join(
                            f"{modality[:3]}_corrupt={mean:.4f}"
                            for modality, _, mean in corrupt_stats
                        )
                        self.logger.info(
                            f"[MMAdj][train@epoch{self._qe_log_epoch}] "
                            f"skip QE-clean graph refresh ({summary})"
                        )
                    return

                for modality, corrupt_mask, _ in corrupt_stats:
                    raw_feat = self._get_modal_repair_raw_feat(modality).detach()
                    graph_feat = raw_feat.clone()
                    graph_feat[corrupt_mask] = repaired_feat_dict[modality].detach()[corrupt_mask]
                    self._set_modal_adj(modality, self._build_mm_adj(graph_feat, None))
        finally:
            if was_training:
                self.train()
        ...
        if hasattr(self, "logger") and self.logger is not None:
            summary = ", ".join(
                f"{modality[:3]}_items={int(mask.sum().item())}"
                for modality, mask, _ in corrupt_stats
            )
            self.logger.info(
                f"[MMAdj][train@epoch{self._qe_log_epoch}] refreshed from QE corrupt-only graph features "
                f"({summary})"
...
class MaskedCrossAttention(nn.Module):
    ...
    def forward(self, query, kv, kv_mask=None):
        ...
        if kv_mask is not None:
            mask = kv_mask.unsqueeze(1)
            attn = attn.masked_fill(~mask, torch.finfo(attn.dtype).min)

        attn = F.softmax(attn, dim=-1)
        if kv_mask is not None:
            attn = attn * kv_mask.unsqueeze(1).to(attn.dtype)
            attn = attn / attn.sum(dim=-1, keepdim=True).clamp_min(1e-8)
        return torch.bmm(attn, v).squeeze(1)


class CleanOnlyNoisyRepairExpert(nn.Module):
    def __init__(self, dim, state_emb_dim):
        super().__init__()
        self.cross_attn = MaskedCrossAttention(dim)
        self.gate_mlp = nn.Sequential(
            nn.Linear(state_emb_dim * 2, 64),
            nn.ReLU(),
            nn.Linear(64, 1),
    ...
    def forward(
        ...
        del target_severity, other_severity

        gate_input = torch.cat([target_state_emb, other_state_emb], dim=-1)
        rho = torch.sigmoid(self.gate_mlp(gate_input))

        fixed_tokens = torch.stack([item_emb, target_hist, id_hist], dim=1)
        fixed_mask = torch.ones(
            target_emb.size(0), fixed_tokens.size(1), dtype=torch.bool, device=target_emb.device
        )
        kv = torch.cat([other_embs, fixed_tokens], dim=1)
        kv_mask = torch.cat([other_mask, fixed_mask], dim=1)

        repair = self.cross_attn(target_emb, kv, kv_mask)
        return self.norm(rho * target_emb + (1.0 - rho) * repair)


class CleanOnlyMissingRepairExpert(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.cross_attn = MaskedCrossAttention(dim)
        self.query_fusion = nn.Sequential(
    ...
    def _masked_mean(self, tokens, mask):
        ...
        denom = weight.sum(dim=1).clamp_min(1e-8)
        return (tokens * weight).sum(dim=1) / denom

    def forward(self, target_emb, other_embs, other_mask, item_emb, target_hist, id_hist):
        del target_hist

        other_summary = self._masked_mean(other_embs, other_mask)
        query = self.query_fusion(torch.cat([other_summary, item_emb], dim=-1))

        anchor_tokens = torch.stack([target_emb, item_emb, id_hist], dim=1)
        anchor_mask = torch.ones(
            target_emb.size(0), anchor_tokens.size(1), dtype=torch.bool, device=target_emb.device
        )
        kv = torch.cat([other_embs, anchor_tokens], dim=1)
        kv_mask = torch.cat([other_mask, anchor_mask], dim=1)

        repair = self.cross_attn(query, kv, kv_mask)
        base = self.base_fusion(torch.cat([id_hist, other_summary], dim=-1))
        alpha = self.blend_gate(torch.cat([repair, base], dim=-1))
        return self.norm(alpha * base + (1.0 - alpha) * repair)


class StateMoEM3V2(StateMoEM3):
    """StateMoEM3 variant with clean-only support from non-target modalities."""
    ...
    def __init__(self, config, dataset):
        ...
        qe_state_dim = self._infer_qe_state_dim(self.img_qe)
        self.img_noisy_expert = CleanOnlyNoisyRepairExpert(
            self.repair_bottleneck, qe_state_dim
        ).to(self.device)
        self.img_missing_expert = CleanOnlyMissingRepairExpert(self.repair_bottleneck).to(
            self.device
        )
        self.txt_noisy_expert = CleanOnlyNoisyRepairExpert(
            self.repair_bottleneck, qe_state_dim
        ).to(self.device)
        self.txt_missing_expert = CleanOnlyMissingRepairExpert(self.repair_bottleneck).to(
            self.device
        )
        self.aud_noisy_expert = CleanOnlyNoisyRepairExpert(
            self.repair_bottleneck, qe_state_dim
        ).to(self.device)
        self.aud_missing_expert = CleanOnlyMissingRepairExpert(self.repair_bottleneck).to(
            self.device
        )

        for expert in (
            self.img_noisy_expert,
            self.img_missing_expert,
            self.txt_noisy_expert,
            self.txt_missing_expert,
            self.aud_noisy_expert,
            self.aud_missing_expert,
        ):
            expert.apply(self.init_weight)

    def _stack_other_modal_tensors(self, tensor_dict, target_modality):
        return torch.stack(
            [tensor_dict[modality] for modality in self.modalities if modality != target_modality],
            dim=1,
        )

    def _stack_other_clean_masks(self, q_dict, target_modality):
        if self.ablation_all_aux_context:
            first_q = next(iter(q_dict.values()))
            return torch.ones(
                first_q["prob"].size(0),
                len(self.modalities) - 1,
                dtype=torch.bool,
                device=first_q["prob"].device,
            )
        return torch.cat(
            [
                self._get_clean_route_mask(q_dict[modality], modality).view(-1, 1)
                for modality in self.modalities
                if modality != target_modality
            ],
            dim=1,
    ...
    def _masked_token_mean(self, tokens, mask):
        weight = mask.to(tokens.dtype).unsqueeze(-1)
        denom = weight.sum(dim=1).clamp_min(1e-8)
        return (tokens * weight).sum(dim=1) / denom

    def _blend_with_item_anchor(self, tokens, weights, item_anchor):
        weight = weights.to(tokens.dtype).unsqueeze(-1)
        numerator = item_anchor + (tokens * weight).sum(dim=1)
        denominator = 1.0 + weights.to(tokens.dtype).sum(dim=1, keepdim=True)
        return numerator / denominator.clamp_min(1e-8)

    def _build_target_repair_context(self, latent_dict, item_anchor, q_dict, target_modality):
        other_embs = self._stack_other_modal_tensors(latent_dict, target_modality)
        other_mask = self._stack_other_clean_masks(q_dict, target_modality)
        other_reliability = self._stack_other_modal_tensors(
            {modality: self._qe_reliability(q_dict[modality]) for modality in self.modalities},
            target_modality,
        ).squeeze(-1)
        other_reliability = other_reliability * other_mask.to(other_reliability.dtype)

        target_hist = self._blend_with_item_anchor(other_embs, other_reliability, item_anchor)

        target_reliability = self._qe_reliability(q_dict[target_modality])
        id_numerator = item_anchor + target_reliability * latent_dict[target_modality]
        id_numerator = id_numerator + (
            other_embs * other_reliability.unsqueeze(-1).to(other_embs.dtype)
        ).sum(dim=1)
        id_denominator = 1.0 + target_reliability + other_reliability.sum(
            dim=1, keepdim=True
        )
        id_hist = id_numerator / id_denominator.clamp_min(1e-8)

        other_state_tokens = self._stack_other_modal_tensors(
            {modality: q_dict[modality]["state_emb"] for modality in self.modalities},
            target_modality,
        )
        other_severity_tokens = self._stack_other_modal_tensors(
            {modality: q_dict[modality]["severity"] for modality in self.modalities},
            target_modality,
        )
        other_q = {
            "state_emb": self._masked_token_mean(other_state_tokens, other_mask),
            "severity": self._masked_token_mean(other_severity_tokens, other_mask),
        }

        return {
            "other_embs": other_embs,
    ...
    def mge(self):
        ...
        for modality in self.modalities:
            keep_expert, noisy_expert, missing_expert = self._get_modal_experts(modality)
            context = self._build_target_repair_context(
                latent_dict, item_anchor, q_dict, modality
            )
            repaired_latent_dict[modality] = self._mix_state_experts(
                target_emb=latent_dict[modality],
                other_embs=context["other_embs"],
                other_mask=context["other_mask"],
                item_emb=item_anchor,
                target_hist=context["target_hist"],
                id_hist=context["id_hist"],
                target_q=q_dict[modality],
                other_q=context["other_q"],
                keep_expert=keep_expert,
                noisy_expert=noisy_expert,
                missing_expert=missing_expert,
                modality=modality,
...
DGMRecStateMoEM3V2 = StateMoEM3V2
